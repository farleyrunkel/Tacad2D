//! class SketchConstraintHorizontal
//! 
//! 
#ifndef SketchConstraintHorizontal_HXX
#define SketchConstraintHorizontal_HXX
...
class SketchConstraintHorizontal : public SketchConstraint
{
public:
    SketchConstraintHorizontal(int lineSegment, 
                             const TDF_Label& label = DocumentTool::SketchConstraintHorizontalsLabel().NewChild())
        : SketchConstraint(label.FindChild(0))
        , myLabel(label)
    {
        TDataStd_Name::Set(label, "SketchConstraintHorizontal");

        SetSegments({lineSegment});
    }

    //! Constructor
    SketchConstraintHorizontal(const TDF_Label& label)
        : SketchConstraint(label.FindChild(0))
        , myLabel(label)
    {
    };
                                                        ...
                                                        const std::vector<int>& segments) 
    {
        ...
        for(size_t i = 0; i < segments.size(); ++i)
        {
            ...
            list.push_back(MakeHandle<SketchConstraintHorizontal>(segments[i]));
...
private:
    ...
    DEFINE_STANDARD_RTTI_INLINE(SketchConstraintHorizontal, SketchConstraint);
};

DEFINE_STANDARD_HANDLE(SketchConstraintHorizontal, SketchConstraint)

#endif  // SketchConstraintHorizontal_HXX
